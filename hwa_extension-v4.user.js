...
(async function() {
    ...
    async function startMainScript(gameCanvas) {
        ...
        let dungeonQuickButtonEl = null
        let macroTimerInterval = null

        function syncDungeonQuickButton() {
            if (!dungeonQuickButtonEl) return
            const active = isRunningMacro === MACRO_DUNGEON
            setActivated(dungeonQuickButtonEl, active, 'STOP', 'RUN')
        }
        ...
        function initUI() {
            ...
            */
            ...
            function buildToolbar() {
                ...
                const dungeonQuickButton = document.createElement('button')
                dungeonQuickButton.id = 'dungeonQuickButton'
                dungeonQuickButton.textContent = 'RUN'
                Object.assign(dungeonQuickButton.style, greenButtonStyle, {
                    minWidth: '80px',
                    width: '80px'
                })

                dungeonQuickButton.onmouseenter = () => {
                    dungeonQuickButton.style.filter = 'brightness(1.12)'
                }

                dungeonQuickButton.onmouseleave = () => {
                    dungeonQuickButton.style.filter = 'brightness(1)'
                }

                dungeonQuickButton.addEventListener('click', async (e) => {
                    e.preventDefault()
                    e.stopPropagation()
                    await runDungeonMacro(false, true)
                })

                dungeonQuickButtonEl = dungeonQuickButton

                //startMacroSession(true)

                return { container, dailyButton, dungeonQuickButton, latestLogEl }
            }
            ...
            // ASSEMBLY
            // ============================================================
            const { container, dailyButton, dungeonQuickButton, latestLogEl } = buildToolbar()
            ...
            container.appendChild(dungeonQuickButton)
            ...
            setActivated(dailyButton, false, BUTTON_TEXT_STOP_CUSTOM, BUTTON_TEXT_RUN_CUSTOM)
            setActivated(dungeonQuickButton, false, 'STOP', 'RUN')
        }
        ...
        // Dungeon MACRO
        async function runDungeonMacro(isResume = false, suppressStopPopup = false) {
            if (isRunningMacro == MACRO_DUNGEON) {
                const summary = getMacroSessionSummary()
                isRunningMacro = null
                setActivated(dailyButton, false, BUTTON_TEXT_STOP_CUSTOM, BUTTON_TEXT_RUN_CUSTOM)
                syncDungeonQuickButton()
                await releaseWakeLock()
                if (!suppressStopPopup && ranLongEnoughForResultsPopup()) {
            ...
            if (fromHomePage) {
                ...
                await runActions([
                    {"x":0.647452, "y":0.907678, actionType: actionClick, title: clickOnGuildTitle},
                ...
                await runActions([
                    ...
                    {"x":0.647452, "y":0.907678, actionType: actionClick, title: "Click on guild"},
                    {pixels: screenGuild, actionType: actionWaitForScreen, delay: DELAY_AFTER_CLICKING_GUILD, title: "Waiting for guild screen"},
...
//                {"x":0.225694,"y":0.312421,"color":[255,62,41], title: "check if head has hp"},
  //              {"x":0.896991,"y":0.897972, title: "auto fight hydra"},
    //            {"x":0.501157,"y":0.892902, title: "confirm fight hydra"},
        ...
        const dungeonQuickButtonSyncTimer = setInterval(() => {
            if (!document.getElementById('dungeonQuickButton')) {
                clearInterval(dungeonQuickButtonSyncTimer)
                return
            }
            syncDungeonQuickButton()
        }, 200)
